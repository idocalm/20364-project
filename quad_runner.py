...
class QuadInterpreter(object):
    ...
    def do_ASN(self, type_, inst):
        self.ns.set(
            ...
            type_,
            inst.opers[0],
            self.val(inst.lineno, type_, inst.opers[1]),
        )
    ...
    def do_EQL(self, type_, inst):
        self.ns.set(
            ...
            inst.opers[0],
            self.val(inst.lineno, type_, inst.opers[1])
            == self.val(inst.lineno, type_, inst.opers[2]),
        )
    ...
    def eval_IASN(self, inst):
        self.do_ASN(int, inst)

    def eval_IPRT(self, inst):
        self.do_PRT(int, inst)

    def eval_IINP(self, inst):
        self.do_INP(int, inst)

    def eval_IEQL(self, inst):
        self.do_EQL(int, inst)

    def eval_INQL(self, inst):
        self.do_NQL(int, inst)

    def eval_ILSS(self, inst):
        self.do_LSS(int, inst)

    def eval_IGRT(self, inst):
        self.do_GRT(int, inst)

    def eval_IADD(self, inst):
        self.do_ADD(int, inst)

    def eval_ISUB(self, inst):
        self.do_SUB(int, inst)

    def eval_IMLT(self, inst):
        self.do_MLT(int, inst)

    def eval_IDIV(self, inst):
        self.do_DIV(int, inst)

    def eval_RASN(self, inst):
        self.do_ASN(float, inst)

    def eval_RPRT(self, inst):
        self.do_PRT(float, inst)

    def eval_RINP(self, inst):
        self.do_INP(float, inst)

    def eval_REQL(self, inst):
        self.do_EQL(float, inst)

    def eval_RNQL(self, inst):
        self.do_NQL(float, inst)

    def eval_RLSS(self, inst):
        self.do_LSS(float, inst)

    def eval_RGRT(self, inst):
        self.do_GRT(float, inst)

    def eval_RADD(self, inst):
        self.do_ADD(float, inst)

    def eval_RSUB(self, inst):
        self.do_SUB(float, inst)

    def eval_RMLT(self, inst):
        self.do_MLT(float, inst)

    def eval_RDIV(self, inst):
        self.do_DIV(float, inst)
    ...
    def eval_JUMP(self, inst):
        if not isinstance(inst.opers[0], int):
            raise QuadError(
                inst.lineno, "invalid instruction number: '{}'".format(inst.opers[0])
            )
